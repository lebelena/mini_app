...
Вы можете передавать свои логин и пароль доверенным лицам. Однако, **доступ к системе также привязан и к профилю телеграм**, из которого выполняется вход. Это означает, что если несколько человек используют одни и те же [учётные данные](*key_data), система будет распознавать их как разных пользователей — по их телеграм-аккаунту.

{% endnote %}

Среди нескольких пользователей одного аккаунта контрагента для дальнейшей работы **выделяется главный и второстепенные пользователи.** Право подтвердить статус главного контрагента предоставляется первому пользователю, выполнившему вход с выданными учётными данными: все последующие пользователи, использующие те же логин и пароль, автоматически получают статус второстепенных. 
